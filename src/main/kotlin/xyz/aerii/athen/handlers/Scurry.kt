...
object Scurry {
    ...
    fun isAreaHovered(x: Float, y: Float, w: Float, h: Float, scaled: Boolean = false): Boolean {
        return if (scaled) Scurry.x in x..(x + w) && Scurry.y in y..(y + h) else rawX in x..(x + w) && rawY in y..(y + h)
    }

    @JvmStatic
    @JvmOverloads
    fun isAreaHovered(x: Float, y: Float, w: Float, scaled: Boolean = false): Boolean {
        return if (scaled) Scurry.x in x..(x + w) && Scurry.y >= y else rawX in x..(x + w) && rawY >= y
    }
}
